//refer this link if you want to understand the sample arduino code:
...
/* update interval */
#define delta_T         100 // in ms

float k[] = {141.421356, -202.001232, 22.137876, 15.965194};
...
void loop()
{
  ...
  uint16_t pendulumArm_Prev = 0;
  uint16_t motorArm_Prev = 0;
  uint16_t pendulumArm;
  uint16_t motorArm;
  float theta_Prev = 0;
  float alpha_Prev = 0;
  ...
  while (1)
  {
    ...
    pendulumArm = getPositionSPI(ENC_0, RES14);
    ...
    while (pendulumArm == 0xFFFF && ++attempts < 3)
    {
      pendulumArm = getPositionSPI(ENC_0, RES14); //try again
    }

    if (pendulumArm == 0xFFFF) //position is bad, let the user know how many times we tried
    {
      Serial.print("pendulumArm/Encoder 0 error. Attempts: ");
    ...
    else //position was good, print to serial stream
    {
      ...
      //      Serial.print("pendulumArm/Encoder 0: ");
      //      Serial.print(pendulumArm, DEC); //print the position in decimal format
      //      Serial.write(NEWLINE);
    }
    ...
    //this function gets the encoder position and returns it as a uint16_t
    //send the function either res12 or res14 for your encoders resolution
    motorArm = getPositionSPI(ENC_1, RES14);

    //if the position returned was 0xFFFF we know that there was an error calculating the checksum
    //make 3 attempts for position. we will pre-increment attempts because we'll use the number later and want an accurate count
    while (motorArm == 0xFFFF && ++attempts < 3)
    {
      motorArm = getPositionSPI(ENC_1, RES14); //try again
    }

    if (motorArm == 0xFFFF) //position is bad, let the user know how many times we tried
    {
      Serial.print("motorArm/Encoder 1 error. Attempts: ");
      Serial.print(motorArm, DEC); //print out the number in decimal format. attempts - 1 is used since we post incremented the loop
      Serial.write(NEWLINE);
    }
    else //position was good, print to serial stream
    {

      //      Serial.print("motorArm/Encoder 1: ");
      //      Serial.print(motorArm, DEC); //print the position in decimal format
      //      Serial.write(NEWLINE);
    }
    ...
    float thetaReading = 3.1416 - ((float)motorArm/16384)*2*3.1416;
    float alphaReading = 3.1416 - (((float)pendulumArm - (float)pendulumArm_Prev) / (16384)) * 2 * 3.1416;
    float thetaRate = (thetaReading - theta_Prev) / delta_T;
    float alphaRate = (alphaReading - alpha_Prev) / delta_T;

    alpha_Prev = alphaReading;

    float control_input = k[0] * thetaReading + k[1] * alphaReading + k[2] * thetaRate + k[3] * alphaRate;
    Serial.print("Theta: ");
    Serial.print(thetaReading);
    Serial.print(", Alpha:");
    Serial.print(alphaReading);
    // Serial.print(", Theta rate: ");
    // Serial.print(thetaRate);
    // Serial.print(", Alpha rate:");
    // Serial.print(alphaRate);
    // Serial.print(", control input:");
    // Serial.println(control_input);
    if (control_input < 0) {
      Serial.print(", control input:");
      Serial.println((uint8_t)(-control_input));
      analogWrite(motorRED, (uint8_t)(-control_input));
      analogWrite(motorBLACK, 0);
    }
    else {
      Serial.print(", control input:");
      Serial.println((uint8_t)(control_input));
      analogWrite(motorRED, 0);
      analogWrite(motorBLACK, (uint8_t)(control_input));
    }

    //    Serial.print("motor arm: ");
    //    Serial.print(motorArm);
    //    Serial.print(", pendulum arm: ");
    //    Serial.print(pendulumArm);
    //    Serial.print(", motor arm prev: ");
    //    Serial.print(motorArm_Prev);

    // delay(delta_T);
    // motorArm_Prev = getPositionSPI(ENC_1, RES14);
